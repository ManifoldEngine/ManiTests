...
// labels
    ...
    {
        constexpr std::string_view MANI_DASHES_STRING = "[--------] ";
        constexpr std::string_view MANI_PASSED_STRING = "[   ok   ] ";
        constexpr std::string_view MANI_FAILED_STRING = "[ FAILED ] ";
        constexpr std::string_view MANI_ASSERT_STRING = "[ ASSERT ] ";
    }
    ...
    namespace _impl
    {
        ...
        struct SimpleTest
        {
            ...
            std::string getLog(const std::string& indent) const
            {
                ...
                if (!hasPassed)
                {
                    testStream << LogColors::BOLD << LogColors::RED << LogConstants::MANI_FAILED_STRING << LogColors::RED << indent << title << ": " << description << LogColors::RESET << "\n";
                    for (const std::string& assertLog : assertLogs)
                    {
                        testStream << LogColors::RED << LogConstants::MANI_ASSERT_STRING << indent << assertLog << LogColors::RESET;
                    }
                }
                else
                {
                    testStream << LogColors::BOLD << LogColors::GREEN << LogConstants::MANI_PASSED_STRING << LogColors::RESET << indent << title << ": " << description << "\n";
                }
        ...
        struct Section
        {
            ...
            std::string getLog(const std::string& indent) const
            {
                ...
                sectionTitle << (hasPassed ? LogColors::GREEN : LogColors::RED) << LogConstants::MANI_DASHES_STRING << LogColors::RESET;
            ...
            static bool runTests(std::vector<SimpleTest>& tests, const std::vector<const Section*>& sectionStack)
            {
                ...
                for (auto& test : tests)
                {
                    ...
                    if (assertLogs.size() > 0)
                    {
                        ...
                        {
                            std::stringstream ss;
                            ss << LogColors::RED << LogConstants::MANI_ASSERT_STRING << assertLogs.front() << LogColors::RESET;
    ...
    inline void run()
    {
        _impl::ManiTestsRunner::runTests();
    }
}
...
#define MANI_TEST(TESTNAME, DESCRIPTION) \
    ...
    static ManiTests::_impl::AutoRegister autoRegister_##TESTNAME(#TESTNAME, DESCRIPTION, TESTNAME);\
    static void TESTNAME()

#define MANI_TEST_ONLY(TESTNAME, DESCRIPTION) \
    static void TESTNAME(); \
    static ManiTests::_impl::AutoRegister autoRegister_##TESTNAME(#TESTNAME, DESCRIPTION, TESTNAME, true);\
    static void TESTNAME()
...
#define MANI_TEST_ASSERT(EXPRESSION, DESCRIPTION) \
    ...
    {\
        ...
        ManiTests::_impl::ManiTestsContext::notifyAssertFailed(ss.str());\
        return;\
    }

#define MANI_SECTION_BEGIN(SECTIONNAME, DESCRIPTION) \
    static ManiTests::_impl::SectionBeginner sectionBeginner_##SECTIONNAME(#SECTIONNAME, DESCRIPTION);\
    namespace

#define MANI_SECTION_BEGIN_ONLY(SECTIONNAME, DESCRIPTION) \
    static ManiTests::_impl::SectionBeginner sectionBeginner_##SECTIONNAME(#SECTIONNAME, DESCRIPTION, true);\
    namespace

#define MANI_SECTION_END(SECTIONNAME) \
    static ManiTests::_impl::SectionEnder sectionEnder_##SECTIONNAME;
    
#define MANI_BEFORE_EACH(FUNCTORNAME) \
    static void FUNCTORNAME(); \
    static ManiTests::_impl::SectionBeforeEachRegister beforeEach_##FUNCTORNAME(FUNCTORNAME); \
    static void FUNCTORNAME()

#define MANI_AFTER_EACH(FUNCTORNAME) \
    static void FUNCTORNAME(); \
    static ManiTests::_impl::SectionAfterEachRegister afterEach_##FUNCTORNAME(FUNCTORNAME); \
    static void FUNCTORNAME()
